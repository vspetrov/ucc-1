...
static inline ucc_schedule_t *ucc_tl_ucp_get_schedule(ucc_coll_args_t *args, ucc_tl_ucp_team_t *team)
{
    ucc_tl_ucp_context_t *ctx      = UCC_TL_UCP_TEAM_CTX(team);
    ucc_schedule_t       *schedule = ucc_mpool_get(&ctx->req_mp);
    ucc_schedule_init(schedule, args, &team->super.super, 0);
    return schedule;
}
...
ucc_tl_ucp_init_task(ucc_base_coll_args_t *coll_args, ucc_base_team_t *team, int n_deps)
{
    ...
    ucc_coll_task_init(&task->super, &coll_args->args, team, n_deps);
