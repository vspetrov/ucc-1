...
static ucc_status_t
ucc_mc_cuda_stream_req_mpool_chunk_malloc(ucc_mpool_t *mp, size_t *size_p, void **chunk_p)
{
    ucc_status_t status;
...
static void ucc_mc_cuda_stream_req_mpool_chunk_free(ucc_mpool_t *mp, void *chunk)
{
    cudaFreeHost(chunk);
}
...
static ucc_status_t ucc_mc_cuda_init()
{
    ...
    status = ucc_mpool_init(&ucc_mc_cuda.events, 0, sizeof(ucc_mc_cuda_event_t),
                            ...
                            &ucc_mc_cuda_event_mpool_ops,
                            UCC_THREAD_SINGLE, "CUDA Event Objects");
    if (status != UCC_OK) {
    ...
    /* create request pool */
    status = ucc_mpool_init(&ucc_mc_cuda.strm_reqs, 0, sizeof(ucc_mc_cuda_stream_request_t),
                            0, UCC_CACHE_LINE_SIZE, 16, UINT_MAX,
                            &ucc_mc_cuda_stream_req_mpool_ops,
                            UCC_THREAD_SINGLE, "CUDA Event Objects");
    if (status != UCC_OK) {
        mc_error(&ucc_mc_cuda.super, "Error to create event pool");
        return status;
