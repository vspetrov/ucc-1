...
#define LINE_SIZE 8 // Could be changed, but in tests performed great
#define NUM_POOLS 2 // We need extactly two pools
            ...
            if (popped_task) {
                if (ucc_atomic_bool_cswap64((uint64_t *) &(pq_mt->tasks[which_pool][i]), (uint64_t) popped_task, 0)) {
                    ...
                    *popped_task_ptr = popped_task;
                    ...
                    i = -1;
                    // TODO - break statement was here on XCCL, seems like a (perf) mistake - we want the pop to give at least 1 task if exist
                    // After the MT is fully tested, this comment can be dropped
                }
            }
        }
    }
...
ucc_status_t ucc_pq_mt_progress(ucc_progress_queue_t *pq) {
    ...
    if (task) {
        ...
        if (UCC_OK == task->super.status) {
            return ucc_event_manager_notify(task, UCC_EVENT_COMPLETED);
        } else {
