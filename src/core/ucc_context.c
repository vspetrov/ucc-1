...
poll:
    if (addr_storage->oob_req) {
        do {
            ...
            if (status < 0) {
                ...
                return status;
            } else if (t_oob && (UCC_INPROGRESS == status)) {
                /* If this function is used from team creation flow
                   then it can't block */
                return status;
            }
    ...
    if (0 == addr_storage->addr_len) {
        if (NULL == addr_storage->storage) {
            ...
            addr_storage->storage = ucc_malloc(addr_storage->size * sizeof(size_t), "max_addrlen_tmp");
            ...
            status = oob->allgather(&context->attr.ctx_addr_len, addr_storage->storage,
                                   sizeof(size_t), oob->coll_info, &addr_storage->oob_req);
            if (UCC_OK != status) {
        ...
        addr_storage->storage = ucc_realloc(addr_storage->storage,
        ...
        memcpy(PTR_OFFSET(addr_storage->storage, max_addrlen * addr_storage->size),
               context->attr.ctx_addr, context->attr.ctx_addr_len);
        status = oob->allgather(PTR_OFFSET(addr_storage->storage, max_addrlen * addr_storage->size),
                               addr_storage->storage,
                               max_addrlen, oob->coll_info, &addr_storage->oob_req);
        if (UCC_OK != status) {
            ucc_error("failed to start oob allgather\n");
        }
        goto poll;
                                ...
                                ucc_context_h *context)
{
    ...
    if (params->mask & UCC_CONTEXT_PARAM_FIELD_OOB) {
        status = ucc_core_addr_exchange(ctx, &ctx->params.oob, NULL, &ctx->addr_storage);
        if (UCC_OK != status) {
